...
local cv_enable = CreateClientConVar("cl_sb_subcategories_enabled", "1", true, false, "Enable spawnmenu subcategories? (Update spawnmenu with spawnmenu_reload command after changing this!)")
...
local function PopulateWeapons(pnlContent, tree, browseNode)
    ...
    for class, _wep in pairs(list.Get("Weapon") or {}) do
        ...
        -- Change check to `simple_base` or `simple_base_scoped`
        if not weapons.IsBasedOn(class, "simple_base") and not weapons.IsBasedOn(class, "simple_base_scoped") then
...
hook.Add("PopulateWeapons", "AddSBWeaponContent", function(pnlContent, tree, browseNode)
